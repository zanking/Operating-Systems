...
void * openInput(char * argv[], int i, queue q){
  FILE* inputfp = NULL; //input file
  char hostname[SBUFSIZE];
  char errorstr[SBUFSIZE];
  char firstipstr[INET6_ADDRSTRLEN];
  char* payload_in; //set payload pointer
...
fprintf(stderr,
  ...
  * 0 to TEST_SIZE-1 */
  ...
  for (t = 0; t< numReqThreads; t++){
    ...
    rc = pthread_create(&(threads[t]), NULL, openInput, (argv[], t, q);
    if (rc){
